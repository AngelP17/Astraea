  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative">
        <div className="absolute left-6 top-0 bottom-0 w-px bg-gradient-to-b from-indigo/50 via-secondary/50 to-tertiary/50" />
            ...
            >
              ...
              <div className="flex-1 border border-white/5 bg-surface-low p-4">
                <div className="flex items-center justify-between">
                  <div className="font-display text-sm font-bold uppercase tracking-tight">{stage.label}</div>
...
function StageDetails({ stageId, result }: { stageId: string; result: PipelineResult }) {
  switch (stageId) {
    case 'ingestion':
      return (
        <div className="mt-2 space-y-1 font-mono text-[10px] text-neutral-400">
          <div>source: <span className="text-indigo">{result.event.source}</span></div>
    ...
    case 'feature':
      ...
      return (
        <div className="mt-2 space-y-1 font-mono text-[10px] text-neutral-400">
          <div>features extracted: <span className="text-indigo">{featureCount}</span></div>
    ...
    case 'model':
      return (
        <div className="mt-2 space-y-1 font-mono text-[10px] text-neutral-400">
          ...
          <div>confidence: <span className="text-indigo">{result.assessment.confidence.toFixed(3)}</span></div>
    ...
    case 'decision':
      return (
        <div className="mt-2 space-y-1 font-mono text-[10px] text-neutral-400">
          ...
          <div>priority_score: <span className="text-indigo">{result.prioritized_case.priority_score.toFixed(3)}</span></div>
